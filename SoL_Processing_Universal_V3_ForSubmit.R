...
#User defined functions to extract labeled PSMs and master protein IDs
...
#User defined functions to process probe-labeled PSMs and peptides from heavy/light isotopic enrichment tag experiments
processPSMs_isotope <- function(uData, modIDs, ptmRS=TRUE){
  ...
  light_psms <- labeled_psms[grep(modIDs[1], labeled_psms[,"Modifications"]),]
  heavy_psms <- labeled_psms[grep(modIDs[2], labeled_psms [,"Modifications"]),]
...
processPeps_isotope <- function(processedPSMs){
  ...
  write.table(uniqueScan_DF, file = paste("uniqueScanDF_LFQmerged_", Sys.Date(), ".txt", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE)
...
#User defined functions to process probe-labeled PSMs and peptides from tmt experiments
processPSMs_TMT <- function(uData, modIDs, train_set_forPred, ptmRS = TRUE, plex = "10plex", quant_level = "MS3"){
...
processPeps_TMT <- function(processedPSMs, modIDs, plex = "10plex"){
...
#User defined function to match Uniprot features to processed peptides
...
udata_path <- file.path("/Users/Woz_1/Documents/Post_Doc/Projects/Site_of_Labeling(SoL)/SoL_TMT-1_HEK293T_CP266-AJ12-32_200uM_30min/SoL_Analysis/")
udata_path <- file.path("/Users/Woz_1/Documents/Post_Doc/Projects/Site_of_Labeling(SoL)/SoL_SoL-8_HEK293T_CP266-AJ3-5-14-18-39_200uM_30min/PSM_Files/")

#Read in experimental data
uData <- read.table(paste(udata_path, "SoL_TMT-1_MS3_Focused_MultiPSM_KvarCvar_QuantCorr_Repeat_040822_PSMs.txt", sep =""), header=TRUE, sep = "\t", stringsAsFactors = FALSE)
uData <- read.table(paste(udata_path, "SoL_SoL-8_AJ14_SoL_MultiPSMSearch_090221_PSMs.txt", sep =""), header=TRUE, sep = "\t", stringsAsFactors = FALSE)

#Set path to metadata
sol_metadata_path <- file.path("/Users/Woz_1/Documents/Post_Doc/Projects/Site_of_Labeling(SoL)/General_Info/sol_processing_metadata/")
...
#Set Mod IDs and plex size (for TMT experiments - 10 or 16plex)
modIDs <- c("-AC-Light", "-AC-Heavy")
plex_size <- "16plex"

#Process SoL data 
processedPSMs <- processPSMs_isotope(uData, modIDs, ptmRS = TRUE)
...
write.csv(processedPSMs, file = paste(udata_path, "SoL_ProcessedPSMs_", Sys.Date(), ".csv", sep = ""), quote = FALSE, row.names = FALSE)
...
write.csv(processedPeps, file = paste(udata_path, "SoL_ProcessedPeptides_", Sys.Date(), ".csv", sep = ""), quote = FALSE, row.names = FALSE)

#####################################################################################
#######                                                                       #######
#######           Match structural features to processed peptides             #######
#######                                                                       #######
#####################################################################################

#Read in structural features from Uniprot and merge with AlphaFold IDs
...
write.csv(processedPeps_wFeatures, file = paste(udata_path, "SoL_ProcessedPeptides_wFeatures_", Sys.Date(), ".csv", sep = ""),  quote = FALSE, row.names = FALSE)
